<template>
  ...
  <div class="camera-button">
      <button  v-if="!isCameraOpen" type="button" class="bg-tfa_primary text-white button is-rounded rounded-full h-24 w-24 flex items-center" :class="{ 'is-primary' : !isCameraOpen, 'is-danger' : isCameraOpen}" @click="toggleCamera">
        Open Camera
  ...
  <div v-if="isCameraOpen" v-show="!isLoading" class="camera-box" :class="{ 'flash' : isShotPhoto }">
    ...
    <div class="camera-shutter" :class="{'flash' : isShotPhoto}"></div>
      ...
      <div  v-show="!isPhotoTaken" class="camera-container">
          <video id="video" v-show="!isPhotoTaken" ref="camera" :width="450" :height="337.5" autoplay></video>
      </div>
    <canvas v-show="isPhotoTaken" class="camera-container"  id="photoTaken" ref="canvas" :width="450" :height="337.5"></canvas>
  </div>
  ...
  <div v-if="isCameraOpen && !isLoading" class="camera-shoot">
    <button :disabled = scaningImage  type="button" class="button" @click="takePhoto">
      <img src="https://img.icons8.com/material-outlined/50/000000/camera--v2.png">
    </button>
          <loader v-if="scaningImage" object="#7A60DB" color1="#2E1C88" color2="#5034BD" size="5" speed="2" bg="#343a40" objectbg="#999793" opacity="0" name="circular"></loader>

    <!-- <div v-else-if="scaningImage" class="camera-loading">
    <ul class="loader-circle">
      <li></li>
      <li></li>
      <li></li>
...
<script>
    ...
    //   console.log(canvas)
      this.axios
        .post(this.$hostname + "api.php?action=detect_image", {image: this.canvas})
        .then((response) => {
        ...
        if(response.data.Errors){
            ...
            if(Object.keys(response.data.images).length == 1){
                ...
                this.axios.post(this.$hostname + "api.php?action=save_image", userData ).then((response)=>{
                    console.log(response)
                    this.scaningImage =false
                        this.toggleCamera()
